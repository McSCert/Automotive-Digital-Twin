...
// C/C++ source code generated on : Fri Mar 21 16:08:52 2025
//
#ifdef _MSC_VER
#pragma warning(push)
...
#include <chrono>
#include <utility>
const std::string SLROSNodeName("data_preprocessing_fe");
...
void NodeInterface::initialize(int argc, char * const argv[]) {
    try {
        ...
        std::string NodeName("data_preprocessing_fe");
        SLROSNodePtr = std::make_shared<rclcpp::Node>(NodeName);
        RCLCPP_INFO(SLROSNodePtr->get_logger(),"** Starting the model \"data_preprocessing_fe\" **\n");
        mExec = std::make_shared<rclcpp::executors::MultiThreadedExecutor>();
        mExec->add_node(SLROSNodePtr);
        //initialize the model which will initialize the publishers and subscribers
		rtmSetErrorStatus(data_preprocessing_fe_M, (NULL));
        data_preprocessing_fe_initialize();
...
boolean_T NodeInterface::getStopRequestedFlag(void) {
    ...
    return (!(rtmGetErrorStatus(data_preprocessing_fe_M)
        == (NULL)));
    #else
    return (!(rtmGetErrorStatus(data_preprocessing_fe_M)
        == (NULL)) || rtmGetStopRequested(data_preprocessing_fe_M));
    #endif
}
...
void NodeInterface::terminate(void) {
    if (mBaseRateThread.get()) {
        ...
        data_preprocessing_fe_terminate();
        mExec.reset();
        SLROSNodePtr.reset();
        rclcpp::shutdown();
    }
...
void NodeInterface::baseRateTask(void) {
  mRunModel = (rtmGetErrorStatus(data_preprocessing_fe_M) ==
              (NULL));
  while (mRunModel) {
    mBaseRateSem.wait();
#ifdef MW_DEBUG_LOG
